...
class BreakpointValue<T> extends BaseLayoutValue<T> {
  ...
  BreakpointValue.fromMap(Map<LayoutBreakpoint, T> values, [T? defaultValue])
      : assert( values.length == LayoutBreakpoint.values.length || defaultValue != null, 'A default value is required if there is not a value asigned to a breakpoint inside the map'),
